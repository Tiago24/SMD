...
: vertices(), material(Material::MT_GOLD), drawModes(TriangleMesh::DRAW_TRIANGLES), aabb(), hasNormals(false)
{
}
...
: vertices(rhs.vertices), material(rhs.material), drawModes(rhs.drawModes), aabb(rhs.aabb), hasNormals(rhs.hasNormals)
{
}
...
Vector3D TriangleMesh::triangleNormal(const Vertex& v0, const Vertex& v1, const Vertex& v2)
{
	return TriangleMesh::triangleNormal(v0.position, v1.position, v2.position);
}

Vector3D TriangleMesh::triangleNormal(const Vector3D& v0, const Vector3D& v1, const Vector3D& v2)
{
